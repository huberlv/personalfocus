/**
 * Farbtastic Color Picker 1.2 Â© 2008 Steven Wittens
 * 
...
jQuery.farbtastic = function(container, callback) {
	...
	return container.farbtastic
			|| (container.farbtastic = new jQuery._farbtastic(container,
					callback));
}
...
jQuery._farbtastic = function(container, callback) {
	...
	fb.updateDisplay = function() {
		...
		$('.h-marker', e).css(
				{
					left : Math.round(Math.sin(angle) * fb.radius + fb.width
							/ 2) + 'px',
					top : Math.round(-Math.cos(angle) * fb.radius + fb.width
							/ 2) + 'px'
				});

		$('.sl-marker', e).css(
				{
					left : Math.round(fb.square * (.5 - fb.hsl[1]) + fb.width
							/ 2) + 'px',
					top : Math.round(fb.square * (.5 - fb.hsl[2]) + fb.width
							/ 2) + 'px'
				});
		...
		if (typeof fb.callback == 'object' && $(fb.callback).attr('id') != '' ) {   //ÐÞ¸Äµ¼º½À¸³¬Á´½ÓÑÕÉ«
		...
		/*
		 * else if (typeof fb.callback == 'function') { fb.callback.call(fb,
		 * fb.color); }
		 */
	};
