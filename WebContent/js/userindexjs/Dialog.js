...
if ( !this.ParentWindow ){
	...
	document.charset="utf-8" ;//设置字符集 简体中文:gb2312
	...
	arr.push("              <tr><td width='25' height='50' align='right'></td>");//放图片
	arr.push("                <td align='center' style='line-height:16px;'>");//放内容
	...
	arr.push("            <div id='_DialogButtons_"+this.ID+"' style='text-align:center; border-top:#dadee5 2px solid; padding:8px 20px; background-color:#f6f6f6;'>");
	
	
	arr.push("           	<input id='_ButtonLarge_"+this.ID+"'  type='button'  value='扩大范围'>");
	arr.push("           	<input id='_ButtonSmall_"+this.ID+"'  type='button'  value='缩小范围'>");
	...
	this.LargeButton =pw.$("_ButtonLarge_"+this.ID);
	this.SmallButton =pw.$("_ButtonSmall_"+this.ID);
	...
	if(this.Large){
		this.LargeButton.onclick = this.Large;
	}
	if(this.Small){
		this.SmallButton.onclick = this.Small;
	}
	if(!this.AlertFlag){
		$E.show(bgdiv);
