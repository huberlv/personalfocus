...
public class Document extends Element {
	 ...
	 * @return
	 *
	public Element mySelect(String path) {
		ArrayList<String> vpath = new ArrayList<String>(10);
		Matcher matcher = this.groupPattern.matcher(path);
		while (matcher.find()) {
			vpath.add(matcher.group(1)); //
			vpath.add(matcher.group(2)); //
		}
		if (vpath.size() < 2) {
			return null;
		}

		return getTagNodeByPath(vpath, this.body());
	}*/
	 ...
	 * @return 给定的特征的节点
	 *
	public Element getTagNodeByPath(ArrayList<String> path, Element tag) {
		try {
			Element currentTag = tag;
			int index = 0;
			Elements children;
			for (int i = 2; i < path.size() - 1; i++) {

				// System.out.println(currentTag.getChildTags().length);
				index = Integer.valueOf(path.get(i + 1));
				children = currentTag.children(path.get(i));
				if (index < children.size()){
					currentTag = currentTag.children(path.get(i)).get(index);
				}else{
					return null;
				}
				i++;
