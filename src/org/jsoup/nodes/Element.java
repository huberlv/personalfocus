...
public class Element extends Node {
     ...
     * @see #childNodes()
     */
    public Elements children(String tagName) {
        // create on the fly rather than maintaining two lists. if gets slow, memoize, and mark dirty on change
        List<Element> elements = new ArrayList<Element>();
        for (Node node : childNodes) {
            if (node instanceof Element){
     ...
     */
    public Elements select(String query) {//lv.hongpei改为
    ...
    public Integer elementSiblingIndex() {
       ...
       return indexInList(this, parent().children(this.tagName())); 
    }
	...
	public String getPath() {
		...
		Elements parents = this.parents(); // 当前节点的祖父辈集合，是一个数组

		String tagName = null;
		Element cParent = null;
		int inde = 0;
		String path ="";
		...
		for (int i = 0; i < parents.size(); i++) {
			tagName = cParent.tagName();
			inde = parents.get(i).children(tagName).indexOf(cParent);// 节点在父节点和其同名儿子集合中的偏移位置
			path = ":eq(" + inde + ")" + path;
			path = tagName + path; // 加入节点名
			cParent = parents.get(i);
			path = ">" + path;
		}
		path = "html" + path;
